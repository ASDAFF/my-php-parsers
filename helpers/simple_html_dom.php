...
function html_restoreXML($filename)
{
	...
	foreach ($xml_offers as $xml_offer) {
		...
		if (count($xml_offer->picture) > 1) {
			$picture = (array)$xml_offer->picture;
		} else {
			$picture = (string)$xml_offer->picture;
		}

		if ($xml_offer->param) {
			$_params = $xml_offer->param;
			foreach ($_params as $_param) {
				$_param_attributes = $_param->attributes();
				$_param_name = addslashes((string)$_param_attributes['name']);
				$_param_value = (string)$_param;
				if (($_param_name && $_param_value) && (mb_strlen($_param_name) < 100)) {
...
function html_getSaitData($url, $userOptions = [])
{
	$ch = curl_init($url);

	if (isset($userOptions['useragent'])) {
		$useragent = $userOptions['useragent'];
	} else {
		$useragent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.89 Safari/537.36';
	}

	if (isset($userOptions['headers'])) {
		curl_setopt($ch, CURLOPT_HTTPHEADER, $userOptions['headers']);
	}

	if(isset($userOptions['data'])) {
		curl_setopt($ch, CURLOPT_POSTFIELDS, $userOptions['data']);
		curl_setopt($ch, CURLOPT_POST, true);
	}

	if(isset($userOptions['timeout'])) {
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $userOptions['timeout']);
		curl_setopt($ch, CURLOPT_TIMEOUT, $userOptions['timeout'] * 3);
	} else {
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 360);
		curl_setopt($ch, CURLOPT_TIMEOUT, 1080);
	}
	...
	if(isset($userOptions['debug']) && curl_errno($ch)) {
...
function html_writeStartFile($outXml, $categories, $companySite, $company, $companyName)
{
	...
	file_put_contents($outXml, $txt);
}

function html_writeProductInFile($outXml, $catId, $product)
{
	$txt = '';

	if ($product) {
		$available = $product['available'] ? 'true' : 'false';
		$group_id = isset($product['group_id']) ? $product['group_id'] : $product['id'];
		$txt .= "      <offer id=\"" . $product['id'] . "\" available=\"" . $available . "\" group_id=\"" . $group_id . "\">\n";
		$txt .= "        <url><![CDATA[" . $product['url'] . "]]></url>\n";
		if (isset($product['currency'])) {
			$txt .= "        <currencyId>" .  $product['currency'] . "</currencyId>\n";
		} else {
			$txt .= "        <currencyId>RUR</currencyId>\n";
		}
		$txt .= "        <categoryId>" . $catId . "</categoryId>\n";
		if (is_array($product['image'])) {
			foreach ($product['image'] as $ival) {
				$txt .= "        <picture>" . $ival . "</picture>\n";
			}
		} else {
		...
		if (isset($product['params']) && is_array($product['params'])) {
			foreach ($product['params'] as $key => $param) {
				$txt .= "        <param name=\"" . $key . "\"><![CDATA[" . $param . "]]></param>\n";
			}
		}
		$txt .= "      </offer>\n";
	}

	file_put_contents($outXml, $txt, FILE_APPEND);
}

function html_writeCuponInFile($outXml, $catId, $product)
{
	$txt = '';
	...
	if ($product) {
		...
		$txt .= "        <categoryId>" . $catId . "</categoryId>\n";
		$txt .= "        <picture>" . $product['picture'] . "</picture>\n";
		$txt .= "        <name><![CDATA[" . $product['name'] . "]]></name>\n";
		$txt .= "        <description><![CDATA[" . $product['description'] . "]]></description>\n";
		$txt .= "        <price><![CDATA[" . $product['price'] . "]]></price>\n";
		if ($product['supplier']) {
			$txt .= "        <supplier>\n";
			$txt .= "           <name><![CDATA[" . $product['supplier']['name'] . "]]></name>\n";
			$txt .= "           <url><![CDATA[" . $product['supplier']['url'] . "]]></url>\n";
			$txt .= "           <tel><![CDATA[" . $product['supplier']['tel'] . "]]></tel>\n";
			$txt .= "        <addresses>\n";
			$txt .= "           <address>\n";
			$txt .= "               <name><![CDATA[" . $product['supplier']['address'] . "]]></name>\n";
			$txt .= "               <coordinates>" . $product['supplier']['coordinates'] . "</coordinates>\n";
			$txt .= "           </address>\n";
			$txt .= "        </addresses>\n";
			$txt .= "        </supplier>\n";
		}
		$txt .= "        <region><![CDATA[" . $product['city'] . "]]></region>\n";
		if (isset($product['conditions'])) {
			$txt .= "        <conditions>" . $product['conditions'] . "</conditions>\n";
		}
		$txt .= "        <discount>" . $product['discount'] . "</discount>\n";
		$txt .= "        <discountprice>" . $product['discountprice'] . "</discountprice>\n";
		$txt .= "        <pricecoupon>" . $product['pricecoupon'] . "</pricecoupon>\n";
		$txt .= "        <beginsell>" . $product['beginsell'] . "</beginsell>\n";
		$txt .= "        <beginvalid>" . $product['beginvalid'] . "</beginvalid>\n";
		$txt .= "        <endsell>" . $product['endsell'] . "</endsell>\n";
		$txt .= "        <endvalid>" . $product['endvalid'] . "</endvalid>\n";
		$txt .= "      </offer>\n";
	}
...
function html_writeEndFile($outXml)
{
	$txt = '';
	$txt .= "    </offers>\n";
	$txt .= "  </shop>\n";
...
function html_clear_var($var, $type = 'string')
{
