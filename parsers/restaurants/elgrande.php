...
$inParceUrl = ["http://www.elgrande.su/"];
$companySite = "http://www.elgrande.su";
$company = "elgrande";
$companyName = null;
if ($companyName == null) {
    $companyName = $company;
...
function index($inParceUrl, $companySite, $company, $companyName)
{
    ...
    // Меняем местами категории "японская кухня" и "десерты"
    $key_1 = key(array_filter($categories, function($innerArray){return ($innerArray['name'] == 'десерты');}));
    $key_2 = key(array_filter($categories, function($innerArray){return ($innerArray['name'] == 'японская кухня');}));
    if ($key_1 && $key_2)
        list($categories[$key_1], $categories[$key_2]) = array($categories[$key_2], $categories[$key_1]);
...
function createProducts($outXml, $urlCatParce, $catId, $baseUrl)
{
    ...
    foreach ($prods_parcing as $html_products) {
        ...
        foreach ($items_parcing as $product) {
            ...
            //Если категория Пицца, то берем второй id за размер - 33см
            ...
            //Если категория Пицца, то берем второе название за размер - 33см
            ...
            //Если категория Пицца, то берем вторую цену за размер - 33см
