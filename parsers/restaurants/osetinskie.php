...
$inParceUrl = "http://osetinskie.ru/";
$companySite = "http://osetinskie.ru";
$company = $companyName = "osetinskie";
...
foreach ($categories as $value) {
    createProducts($outXml, $value['href'] . '?limit=999', $value['id'], $companySite);
}
writeEndFile($outXml);
...
function parcing($url)
{
    ...
    $menu = $html_cat->find('.box-category ul', 0);
    if ($menu) {
        $menu = $menu->children();
    }
...
function createProducts($outXml, $urlCatParce, $catId, $companySite)
{
    ...
    foreach ($prods_parcing as $product) {
        ...
        if ($urlData) {
            $aUrl = clear_var($urlData->href);
        }

        $priceData = $product->find('div.price', 0);
        if ($priceData) {
            $priceDataNew = $priceData->find('.price-new', 0);
            if ($priceDataNew) {
                $price = str_replace("р.", '', $priceDataNew->plaintext);
        ...
        $html_article->find('.product-info', 0);
        ...
        $imgData = $html_article->find('.left .image img', 0);
        if ($imgData) {
            $image = str_replace(' ', '%20', $imgData->getAttribute('src'));
        }
