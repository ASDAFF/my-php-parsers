...
$inParceUrl = "http://hinkalnaya.ru/main/restaurant/rid/19";
$companySite = "http://hinkalnaya.ru";
$company = "hinkalnai1";
...
function parcing($url)
{
    ...
    $menus = $html_cat->find('.nav_menu ul', 0);
    if ($menus) {
...
function createProducts($outXml, $urlCatParce, $catId, $companySite)
{
    ...
    $prods_parcing = $html_prod->find('table.items', 0);
    if ($prods_parcing) {
    ...
    foreach ($prods_parcing as $product) {
        ...
        $jsonData = $product->find('.check-items-boxes', 0);
        $imgData = $product->find('a', 0);
        if ($imgData) {
            $trueImage = $imgData->getAttribute('data-lightbox');

            if ($trueImage)
                $image = html_clear_var($companySite . $imgData->href);
        }

        if (empty($jsonData) || empty($image))
            continue;

        $jsonDataUrl = $jsonData->getAttribute('data-url');
        $pid = $jsonData->getAttribute('data-id');
        $data = json_decode(html_getSaitData($companySite . $jsonDataUrl));
        if (!isset($data->items->$pid))
            continue;

        $price = mb_strtolower(html_clear_var($data->items->$pid->price, 'double'), 'UTF-8');
        $name = mb_strtolower(html_clear_var($data->items->$pid->name), 'UTF-8');
