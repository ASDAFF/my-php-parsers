...
$inParceUrl = "http://angelina.com.ru/";
$companySite = "http://angelina.com.ru";
$company = $companyName = "angelina";
...
function parcing($url)
{
    ...
    $menu = $html_cat->find('ul.nav-main', 0);
    if ($menu) {
        $menu = $menu->children();
    }
    ...
    foreach ($menu as $value) {
        ...
        if ($podmenuInstance) {
            ...
            foreach ($podmenuInstance as $podmenuvalue) {
                ...
                $podmenu_href = html_clear_var($podmenu_a->href);
                $podmenu_name = mb_strtolower(html_clear_var($podmenu_a->plaintext), 'UTF-8');
...
{
    $prods_pagination_parcing = $html->find('div.shop2-pageist a');
...
function createProducts($outXml, $urlCatParce, $catId, $companySite, $pagination = false)
{
    ...
    $prods_parcing = $html_prod->find('#shop2-products', 0);
    if ($prods_parcing) {
    ...
    foreach ($prods_parcing as $product) {
        ...
        $nameData = $product->find('div.name a', 0);
        if ($nameData) {
        ...
        $descriptionData = $html_article->find('div#tabs-1', 0);
        if ($descriptionData) {
